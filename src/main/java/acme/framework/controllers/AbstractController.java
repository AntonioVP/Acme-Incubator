...
public abstract class AbstractController<R extends UserRole, E> {
	...
	public Response<E> doGet(final Request<E> request, final ServiceWrapper<R, E> service) {
		...
		case LIST:
			...
			break;
		case PERFORM:
			// HINT: a PERFORM request using the 'GET' method is served as follows: a)
			// instantiate the query
			// HINT+ parameters; b) unbind them into a fresh model. Note that no errors are
			// expected, but
	...
	private ModelAndView buildRequestedView(final Request<E> request, final Response<E> response) {
		...
		// TODO: errors are not bound if the model is a list, which avoids editing
		// lists.
		if (response.hasErrors()) {
